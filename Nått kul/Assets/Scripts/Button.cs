...
public class Button
{
    ...
    SerialPort sp = new SerialPort("COM5", 9600); //(9600)  Opens a connection between Unity and a Serialport. 
    int Value;
    ...
    void get_data()
    {
        if (sp.IsOpen)
        {
            try
            {
                ...
                //pressed = ArduinoValueToUntiyValue(NewLine); // takes the string with info from arduino and creates an array with w ints that can be used in unity.
    ...
    public bool ArduinoValueToUntiyValue(string String)
    {
    ...
    */
        ...
        //xy_Value[0] = x_Value;
        //xy_Value[1] = y_Value;
        int value = Convert.ToInt32(NewLine);
        if(value ==1 ) {
            return true;
        }
        else
